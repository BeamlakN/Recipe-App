...
import { ArrowLeft, Globe, Utensils, Clock, PlayCircle, CheckCircle2 } from 'lucide-react';
...
function RecipeDetail() {
  ...
  // 2. PARSING LOGIC: Turn the instruction string into an array of steps
  // We split by newline (\r\n or \n) and filter out any blank lines
  const steps = recipe.strInstructions
    .split(/\r?\n/)
    .filter(step => step.trim().length > 5); // Filters out very short lines or empty ones

  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 pb-20">
      {/* Top Navigation */}
      <Link to="/" className="inline-flex items-center gap-2 text-orange-600 font-bold mb-8 hover:text-orange-700 transition-colors bg-orange-50 px-4 py-2 rounded-full">
        <ArrowLeft size={20} /> Back to Discover
      </Link>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        
        {/* Left Column: Image and Meta (4/12) */}
        ...
        {/* Right Column: Title, Ingredients, Steps (7/12) */}
